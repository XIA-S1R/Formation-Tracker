...
namespace ego_planner
{
  ...
  void PolyTrajOptimizer::addPVAGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        // tracking
        if (trackingGradCostP(pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
          gdT(i) += omg * (costp / K + step * gradViolaPt);
          costs(5) += omg * step * costp;  // assuming costs has 6 elements
        }
        // feasibility
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/weight_visibility", wei_visibility_, -1.0);
    ...
    // Subscribe to target odom
    target_sub_ = nh.subscribe("target_odom", 10, &PolyTrajOptimizer::targetCallback, this);
  }
