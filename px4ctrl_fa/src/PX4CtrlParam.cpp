...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
    ...
    read_essential_param(nh, "rc_ctrl_rule", rc_ctrl_rule_);

    if(rc_ctrl_rule_ == 2){ // Japan hand
        rule_pitch_ = 1;
        rule_roll_ = 0;
        rule_throttle_ = 2;
        rule_yaw_ = 3;
    }else if (rc_ctrl_rule_ == 1){ // America hand
        rule_pitch_ = 2;
        rule_roll_ = 0;
        rule_throttle_ = 1;
        rule_yaw_ = 3;
    }else{
        ROS_ERROR("RC ctrl rule numer has not been defined... Shutdown px4ctrl process");
    }
