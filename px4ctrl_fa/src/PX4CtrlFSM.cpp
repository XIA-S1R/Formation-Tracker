...
bool PX4CtrlFSM::toggle_emergency_land() {
    mavros_msgs::SetMode cmd;

    if(state_data.current_state.mode == mavros_msgs::State::MODE_PX4_LAND){
        ROS_WARN_STREAM("[PX4Ctrl] : Already in EMERGENCY LAND mode");
        return true;
    }
...
void PX4CtrlFSM::px4ctrlDataPub() {
    quadrotor_msgs::Px4ctrlData msg;
    switch (state) {
        case MANUAL_CTRL:
            msg.px4_ctrl_mode = 1;
            break;
    ...
    msg.px4_ctrl_state          = state_data.current_state;
    px4ctrl_data_pub_.publish(msg);
}
